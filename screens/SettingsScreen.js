...
const APP_RELEASE_CHANGELOGS = {
  '1.5.260': `【v1.5.260 修复文件操作栏毛玻璃高斯模糊】
🎨 1. 彻底修复多选操作栏毛玻璃高斯模糊效果：
   - 移除底部操作栏容器上的高不透明度背景色，杜绝遮蔽原生 BlurView；
   - GlassView 组件增加底层防护，自动剔除外层 style 中的背景色以防遮挡模糊层；
   - 操作栏质感全面对齐全局悬浮 Dock 导航栏，呈现通透细腻的高斯模糊漫反射。`,
  '1.5.259': `【v1.5.259 重构文件管理多选操作栏与避让悬浮Dock栏】
📁 1. 文件管理多选操作栏布局与层级全面重构：
   - 彻底解决多选文件时底部操作按钮（下载、删除、移动、复制、压缩）被全局悬浮 Dock 导航栏完全遮挡的严重交互问题；
   - 将操作栏重构为现代悬浮胶囊卡片（放置在全局 Dock 栏正上方，间距 10px），并采用 GlassView 毛玻璃立体拟态质感；
   - 顶部控制行新增「退出」多选快捷按钮与高亮计数徽章；
   - 激活多选时动态增加文件列表底部间距（paddingBottom: 220），保证最底部的文件夹与文件滑动时不被遮挡。`,
