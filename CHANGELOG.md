...
## [v1.5.260] - 2026-09-25
> **核心主题**：彻底修复文件管理多选操作栏无毛玻璃高斯模糊的问题，与全局悬浮 Dock 栏保持统一通透磨砂质感。

### 🎨 1. 彻底修复多选操作栏毛玻璃高斯模糊效果
- **根因分析**：
  1. 在 `FilesScreen.js` 的 `bottomBarContainer` 样式中硬编码了高不透明度的 `backgroundColor`（深色 `rgba(30, 41, 59, 0.94)` / 浅色 `rgba(255, 255, 255, 0.95)`），导致外层容器直接渲染了近乎纯白/纯黑的死底色，将底层 `BlurView` 的采样背景与模糊效果彻底封死覆盖；
  2. `GlassView` 核心组件原设计中未对传入 `style` 的 `backgroundColor` 进行剥离，导致外部传入的不透明背景色直接渗透到了容器根 View 上。
- **优化与修复方案**：
  - **核心组件底层防护**：在 `components/GlassView.js` 中自动对外部 `style` 进行安全解构，剔除作用于外层根 View 的 `backgroundColor`，杜绝任何外部样式意外遮盖高斯模糊层；
  - **多选栏磨砂质感对齐**：移除 `FilesScreen.js` 中 `bottomBarContainer` 的背景色，增加 `overflow: 'hidden'`，使其 100% 继承全局 `GlassView` 统一的高斯模糊（`BlurView`）与半透明色相层（`colors.glass.bg`）；
  - **视觉体验升级**：多选操作卡片现在与下方的全局悬浮 Dock 导航栏具备完全一致的高级通透毛玻璃质感，文件列表向上滚动穿过卡片底部时，能清晰呈现细腻的背景虚化模糊漫反射效果。

---

## [v1.5.259] - 2026-09-25
> **核心主题**：重构优化文件管理页面多选操作栏，上浮避让悬浮 Dock 导航栏，杜绝遮挡并升级悬浮拟态视觉；补充列表避让空间。

### 📁 1. 文件管理页面多选操作栏布局与层级全面重构
- **根因分析**：文件管理页面在长按或点击多选模式下，底部批量操作栏（下载、删除、移动、复制、压缩）原采用底部绝对贴底布局（`bottom: 0`），而全局 Dock 栏同样为底部悬浮组件（iOS 距底 24px，Android 距底 14px，高度 62px），导致多选操作按钮与文字被全局 Dock 栏完全覆盖遮挡，用户无法直接点击操作；
- **优化与重构方案**：
  - **精准悬浮层级定位**：将多选操作栏从底部贴边改为悬浮胶囊卡片（`bottom: Platform.OS === 'ios' ? 96 : 86`），严格置于悬浮 Dock 栏上方（间距保持 10px 黄金留白），并将 `zIndex` 提升至 95（高于 Dock 栏的 90）；
  - **毛玻璃拟态质感升级**：采用 `GlassView` 容器（圆角 22px），搭配细腻微边框与双模立体阴影，与全局悬浮 Dock 保持完全一致的现代化设计语言；
  - **新增快捷操作项**：在操作栏顶部信息栏新增「退出」快捷按钮，配合已有的「全选/取消全选」与已选计数徽章，交互更加顺畅直观；
  - **列表滚动避让加固**：当 `multiSelect` 激活时，动态将文件列表底部留白（`paddingBottom`）由 110px 提升至 220px，保证底部最后一个文件或文件夹在向上滑动时能完全露出，绝不被悬浮操作栏遮挡。

---

## [v1.5.258] - 2026-09-23
> **核心主题**：优化 Docker 容器升级体验；彻底清除 GitHub Releases 历史发版日志乱码。

### 🐳 1. 彻底优化 Docker 容器升级体验
- **根因分析**：容器升级拉取较大镜像时，反向代理（如 Cloudflare、Nginx 等）往往会在 60~100 秒触发网关超时而提前切断客户端连接，此时触发了前端硬编码的错误判断，弹出了误导性的“升级超时已超过6分钟”阻断弹窗，严重影响使用体验；
- **优化方案**：
  - 彻底移除打扰性且误导的「镜像拉取已超过6分钟」弹窗；
  - 优化确认弹窗文案，明确提示任务将在服务端后台运行且容器会自动重启；
  - 接入后台静默多级安全轮询刷新机制（3s、8s、15s、30s 自动刷新容器列表），不论网络是否因拉取大镜像提前断连，一旦 Unraid 后台完成新容器启动，界面立即无缝呈现最新版本。

### 📝 2. 全量修复 GitHub Releases 历史发版日志乱码
- **根因分析**：Windows 脚本调用 GitHub API 更新 Release 时因默认字符集未声明 UTF-8 导致中文转码乱码；
- **修复方案**：全量重构请求管道，严格通过 UTF-8 字节流批量重新校准并推送 `v1.3.203` 以及 `v1.5.245` ~ `v1.5.257` 全部历史版本的 Release Notes，彻底告别字符乱码。
...
### 📝 1. 软件更新日志体系全面实时化
- **彻底根治更新日志跨版本滞后问题**：针对用户反馈“更新日志已跨越多个版本未及时同步”的问题，全面重构更新日志联动机制；
- **全链路动态版本日志体系**：
  - 设置中心版本详情弹窗动态根据当前构建版本号（如 v1.5.255）精准匹配实时更新日志；
  - 本地 `CHANGELOG.md`、`SettingsScreen.js` 与 GitHub Release 发版管道保持实时同步，彻底告别旧版占位或滞后文案。

### 🐳 2. 彻底修复 Docker Compose 页面打不开及闪退崩溃
...
### 🎨 3. 彻底清除 Docker、虚拟机、文件管理页面的白色矩形框
...
## [v1.5.253] - 2026-09-23
> **核心主题**：修复虚拟机与文件页面搜索红屏崩溃；设置页面全面清除白色矩形框。

### 🔧 1. 修复虚拟机与文件页面点击搜索闪退红屏
- 补全 `lucide-react-native` 的 `Sparkles` 图标导入，彻底根治停泊舱挂载时抛出的 `ReferenceError: Property 'Sparkles' doesn't exist` 报错。

### 🎨 2. 设置页面全面复原标准卡片
- 将设置页面全部 11 处设置卡片复原为全局标准 `<View style={styles.card}>`，根除 Android ScrollView 内毛玻璃卡片的多层白边与发灰白块；
- 移除电源控制栏（重启/关机）右侧的 `powerActionTag` 边框药丸方块，统一对齐为无边框纯文本操作指示。

---

## [v1.5.252] - 2026-09-21
> **核心主题**：搜索原位微光停泊舱设计；背景沉降与搜索结果彻底解耦；设置页面弹窗全面升级毛玻璃。

### 🚀 1. 搜索框上浮后原位微光停泊舱（Docking Bay）
- 搜索岛起飞置顶后，原位优雅显示出微光停泊待命舱（Dashed Border），彻底杜绝空白空洞。

### 🛡️ 2. 背景全量卡片与搜索结果彻底解耦
- 键入搜索词时，背景沉降虚化的海量容器与虚拟机稳如泰山，仅在置顶搜索框正下方规整浮现匹配结果卡片。
...
## [v1.4.235] - 2026-09-18
> **核心主题**：全面根治二级详情页（CPU/GPU/内存/网络）与反代访问卡死转圈、服务器连不上问题；全量上线 `type=gpu` (<50ms)、`type=cpu` (<60ms)、`type=memory` (<50ms) 极速通道；彻底清除 `ls -l /proc/*/fd/*` 全系统扫句柄的性能毒瘤；前端全页面覆盖在途请求锁（In-flight Lock）防止慢网络并发请求堆死 Unraid PHP-FPM 进程池。

### ⚡ 全面根治二级详情页引发的 Unraid PHP-FPM 进程池耗尽卡死
- **根因深度剖析**：
  1. Unraid 自带的 Web 架构（Nginx + PHP-FPM）仅提供少量工作进程（默认约 5 个）。
  2. 此前当用户打开 GPU、CPU 或内存详情页时，由于缺少隔离的极速分支，后台触发的是全量 `metrics_detail`：连带执行全 CPU 采样、两次 Top 30 全系统进程扫描、全部 Docker 网络与 cgroup 遍历、全量虚拟机 `virsh`，以及在 GPU 模块中对全系统数万个文件句柄的遍历（`ls -l /proc/[0-9]*/fd/*`）与 `intel_gpu_top` 阻塞采样；
  3. 单次请求在 Unraid 上耗时达 2~4 秒，而前端每 1 秒就会发起一次新请求；数秒内 5 个 PHP 工作进程全被占满，导致任何新请求（包括反代网页端访问 `api.php`、App 首页拉取）都被阻塞在队列中，表现为**反代网页一直转圈、App 报连接超时/连不上**。
- **全系极速通道隔离 (`api.php 2026.09.18.10`)**：
  - **`type=gpu` 专属极速通道**：仅采集 GPU 遥测与显卡历史走势，彻底切断所有无关的 CPU/内存/虚拟机/网络全量扫描，单次耗时由 3500ms 暴降至 **<50ms**；
  - **`type=cpu` 专属极速通道**：仅处理 CPU 核心拓扑与 CPU 活跃进程，单次耗时压缩至 **<60ms**；
  - **`type=memory` 专属极速通道**：仅直读 `/proc/meminfo` 与内存活跃进程，单次耗时压缩至 **<50ms**；
  - 与此前已上线的 `type=network` 形成四大二级页面独立通道矩阵，彻底杜绝单页拖垮全服。
- **铲除性能毒瘤与引入智能分级缓存**：
  - 彻底移除 `ls -l /proc/[0-9]*/fd/*`，改用设备节点靶向扫描（`fuser` / `lsof`）与媒体进程按需扫描，将活跃程序侦测从 3000ms 降至毫秒级；
  - `intel_gpu_top` 采样超时由 2.0s 严格优化为 0.3s 并采用 50ms 采样步长；
  - GPU 活跃程序引入 2.5s 内存缓存，GPU 遥测引入 1.0s 内存缓存，Docker 容器信息引入 5.0s 内存缓存，避免高并发重复打满系统命令。

### 🛡️ 前端全屏部署「在途请求锁（In-flight Request Lock）」
- **彻底杜绝请求堆叠**：
  - 在 `GpuDetailsScreen`、`CpuDetailsScreen`、`MemoryDetailsScreen`、`NetworkDetailsScreen` 以及首页 `DashboardScreen` 中全量引入 `useRef` 在途锁；
  - 定时器触发时，若前一轮请求因网络延迟或反代握手尚未返回，**绝对不发起新的重叠请求**；
  - 彻底解决了在弱网、移动宽带或反代环境下，请求滚雪球式堵塞 Unraid 后端进程池的严重缺陷。

---

## [v1.4.234] - 2026-09-18
> **核心主题**：彻底根治 Docker 容器详情与名称获取失败问题（不再过滤删除，而是通过 docker ps 权威解析真实容器名）；彻底消除 1113 MB/s 异常尖峰（锁定 br0 单一基准源，消除双网卡交替差值漏洞）；图表与全量模块统一上传在前。

### 🐳 Docker 容器名称与网络详情完美解析（告别 `--` 与空容器）
- **根因分析**：旧版本单纯依赖 `docker stats` 输出，但许多 Docker 版本或 Unraid 环境在 `docker stats` 的第一列中并未注入有效容器名（直接输出 `--` 或容器短 ID）。此前直接跳过 `--` 导致容器被错误全部过滤；
- **权威映射架构 (`get_dockers_network_stats`)**：
  1. 采用极速轻量的 `docker ps -a` (<30ms) 建立权威容器 ID (长/短/全名) 到清晰别名的字典映射表；
  2. 无论 `docker stats` 输出的列是 ID 还是 `--`，均由底层映射解析出容器的真实注册名称（如 plex, qbittorrent 等）；
  3. 即使个别容器使用 Host 共享网络或 `docker stats` 瞬时超时，依然保留容器并在卡片中展示真实名称与运行标识，永不丢失容器；
  4. 引入健壮缓存防抖，避免高频刷新导致 `docker stats` 命令拥塞。

### ⚡ 彻底消除网络走势 1113 MB/s 异常尖峰
- **根因剖析**：Unraid 的网桥 `br0` 包含了 `eth0`，此前由于 `/sys/class/net/br0/brif` 读取偶发竞争，导致前一秒统计仅含 `br0` (33GB)，后一秒错误把 `eth0` 再次相加 (66GB)，瞬间产生 33GB 的巨大差值并被封顶为 1113 MB/s；
- **单一权威单调基准**：优化 `calculate_host_net_totals`，只要 `br0` 存在即以 `br0` 为唯一宿主机吞吐源，严禁在网桥存在时重复叠加物理网卡；
- **异常差值丢弃防御**：在 `action=status` 及 `metrics_detail` 所有计算分支中增加物理网速限幅守卫，当单秒差值异常超出物理上限时直接置零，杜绝任何尖峰写入时序历史；
- **历史缓冲区深度清洗**：读取时自动剔除超过 150 MB/s 的历史脏数据，前端自适应阈值同步降至 150 MB/s，走势图垂直坐标轴彻底恢复至真实正常刻度。

### ⬆️ 图表与指标全面统一“上传在前、下载在后”
...
## [v1.4.233] - 2026-09-18
> **核心主题**：历史缓冲区脏数据彻底清洗（峰值/均值异常根治）；图表纵坐标标签；全屏1秒刷新；Docker容器名称解析容错；网络信息顺序统一上传在前。

### 🔧 网络峰值/均值彻底根治（历史缓冲区过滤）
- **`update_metrics_history()` 双重防护**：读取 `/tmp/unraid_metrics_history.json` 时，自动过滤掉所有 `rx > 1.25 GB/s` 或 `tx > 1.25 GB/s` 的脏条目（启动时零基线误差产生的天文数字）；写入新条目前使用 `min($val, MAX_SANITY_BPS)` 封顶，确保历史缓冲区永远干净；
- **快速路径（fast-path）零基线防护**：`handle_metrics_detail type=network` 中的宿主机实时速率计算增加 `lastRx > 0` 零基线守卫，并添加 1.25 GB/s 上限封顶，防止服务器重启后首次计算产生巨大差值；
- **前端 SANITY 过滤**：`NetworkDetailsScreen.js` 的 `rxPoints/txPoints` useMemo 已过滤超限值（v1.4.232 已有，本版保留）。

### 📊 图表纵坐标标签（Y轴读图）
- `MetricsLineChart.js`：在每条网格水平线旁增加 SVG Text 标签，显示对应的百分比/速率值，方便直接读图无需猜测。

### 🐳 Docker容器名称解析容错
- `api.php`：在 `docker stats` 输出解析循环中增加名称有效性校验，跳过 `name` 为空、`--`、`NAME`（表头行）或长度为零的行，避免无效条目出现在网络详情页容器列表中。

### ⬆️ 网络信息上传/下行顺序统一
- 所有网络相关显示位置统一为**上传（↑）在前，下载（↓）在后**，与用户操作习惯一致：
  - 网络详情顶部大数字卡片：↑上行速率 | ↓下行速率；
  - 流量统计胶囊：累计发送（上传）| 累计接收（下载）；
  - 24h 徽标：↑ 上传 · ↓ 下载；
  - 各容器/虚拟机/进程卡片实时速度：实时上传 | 实时下载；
  - 卡片 24h 累计：↑ 上传 ↓ 下载。

### ⏱️ 全屏1秒刷新频率
- 所有详情页（网络/CPU/内存/GPU/存储/首页）的数据拉取间隔统一设为 1000ms。

---

## [v1.4.232] - 2026-09-18
> **核心主题**：彻底修复网络详情页峰值与均值统计误差（单位一致性、网卡防重复统计、时序图无偏均值计算）；显著恢复并强化宿主机累计上传下载展示；全面支持容器、虚拟机、系统进程 24 小时滚动累计流量统计与卡片徽标。

### 🎯 网络详情页峰值与均值统计彻底校准与修复
- **根因分析与全面治理**：
  1. **单位制混乱消除（1024倍跨度错位）**：此前首页向网络详情页传递的 `netSpeed` 单位为 KB/s，而网络详情页与后端接口均以 Bytes/s（B/s）作为基础单位。当初始速率（如 500 KB/s）未经转换传入时，前端格式化函数误当成 500 B/s（渲染为 0 KB/s），后续接口返回真实现率（512,000 B/s）时导致极大极小值剧烈跳变，严重污染走势图；
  2. **网卡接口双重重复叠加**：在 Linux/Unraid 系统中，默认桥接网卡 `br0` 将物理网卡 `eth0` 包含在内。此前后端统计时同时累加了物理网卡与 `br0`，导致网卡聚合总流量与总吞吐瞬间翻倍；
  3. **平滑贝塞尔时序填充点污染统计**：`MetricsLineChart.js` 此前在绘制曲线时，为了保证贝塞尔平滑度自动向数组前端填充了 15~30 个首值或零值，而峰值与均值统计此前误在填充后的数组上计算，导致均值严重偏小；
- **全链路校准方案**：
  - **接口与页面统一为 Bytes/s**：首页跳转网络详情页时显式转换为 Bytes/s，网络详情页增加单位防御性校验与自适应修正；
  - **后端网卡智能防重叠过滤**：自动读取 `/sys/class/net/br0/brif/` 与 bond 网卡从属关系，精准统计真实独立物理流速与桥接网卡，彻底消除流量翻倍计算；
  - **曲线组件统计算法纯净化**：`MetricsLineChart.js` 中的 `max`（峰值）与 `avg`（均值）严格从过滤 NaN / 有限数值后的真实采样点集计算，与仅用于 SVG 渲染的填充逻辑彻底解耦，峰值与均值 100% 真实反映网络流速。

### 📊 宿主机累计流量展示深度优化与强化
- **恢复并强化累计上传/下载专属模块**：在网络详情页顶部卡片的实时吞吐下方，新增专门的【流量统计】区域；
- **双独立高亮胶囊徽标**：
  - 累计接收（下载）：使用绿色胶囊卡片清晰呈现历史总接收流量；
  - 累计发送（上传）：使用紫色胶囊卡片清晰呈现历史总发送流量；
  - 顶栏副标同步显示 24h 宿主机滚动吞吐统计。

### 🕒 容器、虚拟机、系统进程 24 小时滚动累计流量追踪
- **后端 `api.php` (2026.09.18.06) 24h 遥测累加器**：
  - 在 `/tmp/unraid_net_24h.json` 中建立非阻塞的 24 小时时间窗口环形缓存与单调递增计数器（Monotonic Counter）；
  - 智能感知容器/虚拟机重启或计数器归零重置事件（Reset Detection），自愈累计 delta 差值；
  - 针对 Docker 容器、KVM 虚拟机、活跃系统网络进程（按进程名聚合）持续生成精确的 `rx_24h_bytes` 与 `tx_24h_bytes`；
- **移动端全类型卡片新增 24h 累计徽标**：
  - 每个卡片专属新增【24h累计】栏：展示 `24h累计: ↓ {下载量} · ↑ {上传量} (合计 {总量})`；
  - 完整保留并展示生命周期总累计信息（容器总累计、虚拟机运行状态与总累计、进程总 I/O 与连接数详情）。
...
### ⚡ 彻底修复网络二级详情页不刷新与下拉刷新问题
- **根因分析与全链路排查**：
  1. **后端时间戳未初始化**：`handle_metrics_detail()` 中缺失 `$nowFloat = microtime(true);` 变量初始化，导致与上次采样的差分时间 $\Delta t \le 0$，差分速率算法全部判定无效；
  2. **整机流速与时序缓冲未更新**：`handle_metrics_detail()` 未计算整机实时吞吐，未调用 `update_metrics_history`，且返回体中缺失 `network_speed` 字段，导致时序图和顶栏流速永久停留在静态切片；
  3. **数据结构兼容性隐患**：若接口自动回退至 `action=status`，由于该接口的 `dockers` 与 `vms` 返回包含元信息的对象结构（`{ running, total, list }`），前端 `Array.isArray` 校验不通过导致列表为空；
  4. **请求堆叠与进程阻塞**：高频 2 秒轮询在未受互斥保护的情况下并发堆积，导致 PHP-FPM 工作进程排队阻塞，下拉刷新因而无响应。
- **全量修复与性能优化**：
  - **`api.php` (2026.09.18.05) 上线 `type=network` 极速通道**：专门为网络详情页剔除无关的 CPU/内存多进程扫描与高耗时 GPU 命令，单次请求响应时间骤降至 100~150ms，彻底消除服务端压力；
  - **补齐整机流速与时序更新**：每次采样精准差分宿主机网卡总流速，更新 `/tmp/unraid_metrics_history.json` 并在顶层同时输出 `network_speed` 与 `network`（`rx_bps`/`tx_bps`）；
  - **移动端多源数据格式全兼容**：同时适配扁平数组与嵌套 `list` 结构，无论命中何种数据源均能正确提取并渲染容器与虚拟机；
  - **独立 `currentSpeeds` 响应式状态**：摆脱历史末项依赖，每次网络回包毫秒级更新顶栏瞬时流速；
  - **防请求堆叠锁与下拉刷新保障**：引入 `isFetchingRef` 锁防止轮询积压，下拉刷新增加 `nocache=1` 参数并异步等待刷新完成，双端适配 `RefreshControl`（`colors` 与 `tintColor`）。

### 🎨 首页 2x2 Bento 矩阵交互与布局优化
- **内存与 GPU 位置互换**：
  - **第一行 (Row 1)**：`[ CPU Bento 卡片 ]` + `[ GPU Bento 卡片 ]`；
  - **第二行 (Row 2)**：`[ RAM 内存 Bento 卡片 ]` + `[ 网络 Bento 卡片 ]`；
- **首页网络 Bento 卡片精细化重构**：
  - **移除右上角下载流速标签**：头部保留网络图标与标题，视觉更显简洁大气；
  - **下部流速顺序重排**：严格按用户要求调整为**先上行（紫色 `ArrowUp`），后下行（绿色 `ArrowDown`）**；
  - **无缝衔接二级页面**：点击网络卡片时将当前网络速率与历史走势瞬时同步至二级页面。

---

## [v1.4.229] - 2026-09-18
> **核心主题**：网络二级详情页全面重构升级：上线各个 Docker 容器、虚拟机及系统进程的独立实时上传与下载双向速率监控；支持「全部 / Docker / 虚拟机 / 系统进程」智能分类筛选与总流速排行；后端 `api.php` 升级至 2026.09.18.04 实现容器网络、虚拟机 Tap 网卡与网络套接字进程的实时流速差分测量。

### 🚀 网络二级详情页：容器、虚拟机与进程实时网络速率监控
- **用户需求**：在网络的二级页面中在此基础上加上各个容器，虚拟机，进程等的当前网络占用情况，就是他们各自的当前上传下载的实时速度。
- **分类筛选切换栏 (Segmented Filter Tabs)**：
  - 新增「全部」、「Docker 容器」、「虚拟机」、「系统进程」四个无缝切换选项卡；
  - 动态显示当前筛选分类下的活跃网络程序总数。
- **独立双向流速清晰呈现**：
  - 每个卡片独立展示 **实时下载**（`ArrowDown` 绿色图标 + 速率，如 `12.5 MB/s`）与 **实时上传**（`ArrowUp` 紫色图标 + 速率，如 `2.1 MB/s`）；
  - 右上角配备醒目的当前瞬时总带宽胶囊（`MB/s` / `KB/s`）；
  - 带有动态流速占比能量条，根据当前活跃程序的最大流速自适应展现占用强度；
  - 副标题清晰呈现端口映射、网络模式或对应 PID / 进程路径。
- **智能总流速排序**：
  - 列表严格按照瞬时总流速（下载 + 上传）降序动态排序，正在大流量下载或做种推流的程序即刻浮动至最顶端。

### ⚡ 后端核心 API 升级 (`api.php` 2026.09.18.04)
- **Docker 容器实时流速差分引擎**：
  - 读取 `docker stats` 中的 `net_rx_bytes` 与 `net_tx_bytes`，在 `/tmp/unraid_dockers_net_last.json` 中保存瞬时快照与纳秒级时间戳；
  - 精确计算每个容器的实时下行速率 `net_rx_bps` 与上行速率 `net_tx_bps`。
- **KVM 虚拟机虚拟网卡流量捕获**：
  - 通过 `virsh domiflist` 动态枚举运行中虚拟机的虚拟网卡设备（如 `vnet0`, `vnet1`）；
  - 直接读取 Linux `/sys/class/net/{vnet}/statistics/` 内核网络统计；
  - 自动通过 `/tmp/unraid_vms_net_last.json` 差分得出虚拟机内部操作系统的实时上传下载流速。
- **系统网络进程实时监测 (`get_network_top_processes`)**：
  - 通过 `ss -ntu -p` 扫描当前持有 ESTABLISHED / LISTEN 网络连接的进程 PID；
  - 结合 Linux `/proc/[pid]/io` 统计差分计算该进程的实时网络读写吞吐，捕获如 Samba (`smbd`)、Nginx、WireGuard、qBittorrent 等进程的网络活跃度。
- **移动端前端容错双保险 (`prevNetRef`)**：
  - 即使后端处于冷启动或网络接口计数器单次请求无历史比对，前端也会基于 2 秒轮询周期的前后两次流量差本地兜底计算瞬时速率，确保 100% 显示精准非零的实时流速。
...
### 📁 预览缓存物理落地用户指定下载目录的 `temp/` 文件夹
- **问题反馈**：
  - 用户反馈：「另外软件的缓存文件放在了哪里，我不是说要放到我指定的下载目录下的temp文件夹中吗？怎么没有啊？」。
- **根因分析**：
  - 此前 `cacheManager.js` 仅将缓存固化在 Android 私有沙盒 `FileSystem.documentDirectory + 'preview_cache/'`，在手机系统文件管理器中完全隐形不可见，与用户指定的下载目录彻底脱节。
- **架构级解决方案**：
  - **动态联动指定目录**：全面读取用户在【设置 -> 本地存储与下载】中授权的下载目录（无论是 Android SAF `content://` 授权目录，还是普通文件系统目录）。
  - **自动创建并维护 `temp/`**：通过 StorageAccessFramework 或文件系统在该目录下物理创建并维护 `temp/` 子文件夹。
  - **双向物理镜像保存**：所有预览文件（PDF、Word、Excel、电子书、压缩包等）在高速下载至临时渲染沙盒的同时，立即通过 `saveToCache` 写入用户指定下载目录下的 `temp/` 文件夹中。用户使用系统文件管理器打开下载目录，即可直观看到 `temp/` 及其中的缓存文件。
  - **精确清理与占用统计**：`getCacheFiles()`、`getCacheSize()`、`clearCache()` 与 LRU 上限轮转均以该 `temp/` 文件夹为核算基准；并在设置页面直观展示当前缓存真实落盘路径（例如 `Download/temp/`）。

### ⚡ 彻底根治 GPU 占用率死锁在 95%（消除频率算式、引入真实引擎采样）
- **问题反馈**：
  - 用户反馈：「GPU 刷新的问题还是没有解决，占用率一直卡在 95% 不动，重新弄一下 GPU 信息获取的问题」。
- **根因曝光**：
  - 在 `api.php` 的 Intel 核显分支中，此前存在伪占用率算式：`$usage = round((($curFreq - $minFreq) / ($maxFreq - $minFreq)) * 100, 1)`。
  - 在 Intel iGPU（如 UHD 630 / UHD 770 / Iris Xe）系统上，Linux 内核在插电或启动后默认将当前频率 `gt_cur_freq_mhz` 维持在 ~1150MHz（`min=350`, `max=1200`），该算式计算 `(1150-350)/(1200-350) = 800/850 = 94.1% -> 95%`！因此只要服务器待机，占用率就永远死锁在 95%！
- **修复方案**：
  - **彻底粉碎频率假算式**：绝不将频率比率当做 GPU 占用率！
  - **优先读取真实利用率**：
    - 检查 Unraid `gpustatus.php` / `gpustatusmulti.php`，直接提取插件汇总的真实引擎负载。
    - 运行 `intel_gpu_top -J -s 120`，解析 `Render/3D`、`Video`、`VideoEnhance` 等引擎的真实繁忙度，多引擎取最大峰值。
  - **待机空载如实汇报 0%**：若无活跃编解码/渲染任务，明确汇报 `usage = 0`（待机空载），并准确展示物理时钟频率（如 `1150 MHz`），真正做到与 CPU/内存同步实时起伏。

### 📚 彻底根治 TXT 预览 HTTP 500 与全自动静默同步服务端 API
- **问题反馈**：
  - 用户反馈：「txt 预览还是显示文本读取未响应 HTTP 500」。
- **根因分析**：
  - 服务端此前未被热更新至最新的 `api.php`，仍停留在旧版本的 PHP 8 `mb_convert_encoding` 参数类型异常中；
  - 之前默认读取 2MB 文本，转换为包含转义序列的超大 JSON 导致 Nginx FastCGI 缓冲区溢出；
  - 前端吃掉了 HTTP 500 的真实错误返回，只向用户展现干瘪的“HTTP 500”。
- **全面修复**：
  - **开机/接入全自动静默推更**：在 `DashboardScreen.js` 增加开机静默 API 版本比对机制。一旦检测到服务端版本低于 `BUNDLED_API_VERSION`（`2026.09.17.08`），立即静默推送更新服务端 `api.php`，用户无需手动操作！
  - **设置界面升级指示**：在【设置 -> 后端 API 核心版本】中提供醒目的琥珀色「有新版可更新」角标与「一键升级 API」按钮。
  - **512KB 分片极速秒开**：默认文本分片调整为 512KB（约 25 万字/2万行），毫秒级完成解码与网络传输，彻底规避 FastCGI 缓冲区溢出，提供「+512KB 下一段」丝滑续读。
  - **错误体透出展示**：`CodeTextViewer.js` 支持在请求失败时解析并展示后端详细错误原因，不再盲目抛出白板 HTTP 500。

### 📄 原生 PdfRendererModule 全面兼容 SAF `content://` 与标准文件路径
- 在 `setup-pdf-renderer.js` 构建注入脚本中，为原生 `PdfRendererModule.java` 增加 `openPfd` 方法，无缝通过 Android `ContentResolver` 打开 `content://` 协议的 PDF 文件描述符，支持从任意系统目录或沙盒中免服务依赖硬件级离线渲染。
