...
| 📁 **文件浏览器** | 基于 WebDAV（PROPFIND）的文件管理：浏览、上传、下载、删除、**长按操作菜单（重命名/复制/移动）**、**多选批量操作**、图片预览，兼容 AList 的 `/dav/` 路径 |
| 🎨 **主题切换** | 全局深色 / 浅色主题一键切换，即时生效并自动保存 |
| ⚙️ **系统设置** | 服务器连接管理与注销、深色模式切换、缓存清理 |
...
- 在任意文件或文件夹上**长按**，会弹出底部操作菜单，可对当前项执行：**下载**、**重命名**、**复制**、**移动**、**删除**。

**多选批量操作**

- 每个文件 / 文件夹**左侧有复选框**，点击可进入多选模式并勾选。
- 多选模式下，**页面底部**会出现操作栏：**移动**、**复制**、**删除**，支持全选 / 取消全选。
- 复制 / 移动会打开**目标目录选择器**，可浏览服务器目录树选择目标位置后批量执行。

### 5. 系统设置（设置 Tab）

- **主控服务器**：查看当前连接地址，或「注销并重新配置」
- **外观与个性化**：全局**深色 / 浅色主题**切换，即时生效并自动保存偏好
- **存储管理**：查看本地缓存占用，一键清理缓存
- 底部显示应用当前版本号
